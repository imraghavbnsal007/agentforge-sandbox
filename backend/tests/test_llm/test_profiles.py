...
def test_builtin_profiles_match_spec():
    ...
    assert profiles["cheap"]["planning"] == ModelSpec("google", "gemini-3.1-flash-lite")
    assert profiles["balanced"]["analysis"] == ModelSpec("google", "gemini-3.1-flash-lite")
...
def test_resolution_precedence(monkeypatch: pytest.MonkeyPatch):
    ...
    assert specs["coding"] == ModelSpec("google", "gemini-3.1-flash-lite")
